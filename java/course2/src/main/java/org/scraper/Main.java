...
public class Main {
  public static void main(String[] args) {
    try {
      ...
      String[] services = args[2].split(" ");//Arrays.copyOfRange(args, 2, args.length - 1); // length = thread number
      ...
      Semaphore semaphore = new Semaphore(threshold);
        for (String api : services) {
          pollers.scheduleAtFixedRate(new ApiPoller(new ApiServiceModel(api), dataWriter, semaphore),
      ...
      pollers.shutdownNow();
      dataWriter.close();
    } catch ( IllegalArgumentException | InterruptedException | IOException | NullPointerException e) {
